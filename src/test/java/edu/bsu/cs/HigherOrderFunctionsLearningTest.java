...
public class HigherOrderFunctionsLearningTest {
    ...
    public void testMap() {
        ...
        List<Integer> actual = input.map(n -> n + 1)
                .collect(Collectors.toList()); // TODO: Replace the `actual` with your solution, here and in all the following.
    ...
    public void testFilter() {
        ...
        List<String> actual = input.filter(s -> s.startsWith("b"))
                .collect(Collectors.toList());
    ...
    public void testDistinct() {
        Stream<String> input = Stream.of("a1", "a2", "a3", "b1", "b2", "b3");
        List<String> actual = input.map(n -> n.substring(0,1))
                .distinct()
                .collect(Collectors.toList());
    ...
    public void testConcatEven() {
        Stream<String> input = Stream.of("a1", "a2", "a3", "b1", "b2", "b3");
        String actual = input.filter(s -> ((int) s.charAt(1)) % 2 == 0)
    ...
    public void testCountByDate() {
        ...
        long actual = input.filter(s -> s.timestamp.isBefore(instant))
                .count();
        int expected = 3;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountWhitelisted() {
        ...
        long actual = input.filter(s -> whitelist.contains(s.user))
                .count();
        int expected = 3;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountNonBots() {
        Stream<Revision> input = getRevisions("soup30.json");
        long actual = input.filter(s -> !s.user.toLowerCase().contains("bot"))
                .count();
        int expected = 26;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountChangesByMonth() {
        Stream<Revision> input = getRevisions("soup04.json");
        Map<Month, Long> actual = null;/*input.collect(Collectors.toMap(
                p -> p.timestamp.atZone(ZoneOffset.UTC).getMonth(),
                p -> p.timestamp.atZone(ZoneOffset.UTC).getMonth().equals("")).count()
        );*/
